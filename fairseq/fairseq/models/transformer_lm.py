...
class TransformerLanguageModelConfig(FairseqDataclass):
    ...
    word_dropout_mixture: Optional[float] = field(
        ...
        metadata={"help": "Proportion of the time to use word dropout "
                          "objective instead of full sequence objective. For "
                          "word dropout, the model first samples a sequence "
                          "length uniformly at random and then considers all "
                          "sequences uniformly at random given the length."}
    )
...
class TransformerLanguageModel(FairseqLanguageModel):
    ...
    def build_model(cls, args, task):
        ...
        embed_year = cls.build_embedding(
            args, task._year_dictionary, args.decoder_input_dim)
        embed_education = cls.build_embedding(
            args, task._education_dictionary, args.decoder_input_dim)
        embed_ethnicity = cls.build_embedding(
            args, task._ethnicity_dictionary, args.decoder_input_dim)
...
@register_model_architecture("transformer_lm", "career")
def career(args):
