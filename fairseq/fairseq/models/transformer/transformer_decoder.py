...
class TransformerDecoderBase(FairseqIncrementalDecoder):
    ...
    def __init__(
        ...
        # added for CAREER (for rationalization):
    ...
    def build_output_projection(self, cfg, dictionary, embed_tokens):
        if cfg.adaptive_softmax_cutoff is not None:
            ...
            # If making predictions in two stages, there is one extra output
            # class (whether or not the job repeats).
            num_output_classes = (
                len(dictionary) + 1 if cfg.two_stage else len(dictionary)
            )
            self.output_projection = nn.Linear(
                self.output_embed_dim, num_output_classes, bias=False
            )
            nn.init.normal_(
    ...
    def extract_features_scriptable(
        ...
        if (self.training and 
            ...
            num_tokens_to_keep = torch.randint(
              low=0, high=tgt_len + 1, size=[1])
            self_attn_word_dropout_mask[
              :, candidates[:num_tokens_to_keep]] = 1.
            # To avoid nan errors, use -1e4 instead of -inf.
